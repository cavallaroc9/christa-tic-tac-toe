...
var displayGameStat = function displayGameStat() {
  api.index().then(ui.getGamesSuccess).catch(ui.getGamesFailure);
};

var onCreateGame = function onCreateGame(event) {
  api.create().then(ui.createBoardSuccess).then(resetBoard()).catch(ui.createBoardFailure);
};

var displayDraw = function displayDraw(event) {
  $('#win-or-draw').show();
...
var updateDraw = function updateDraw(event) {
  over = true;
  var game = {
    'game': {
      'cell': {
  ...
  api.update(game).then(ui.updateBoardSuccess).then(displayDraw()).catch(ui.updateBoardFailure);
};

var isDraw = function isDraw(cellsArray) {
  return cellsArray !== '';
};
...
var updateWinner = function updateWinner(event) {
  over = true;
  var game = {
    'game': {
      'cell': {
  ...
  api.update(game).then(ui.updateBoardSuccess).then(displayWinner()).catch(ui.updateBoardFailure);
};

var findWinner = function findWinner(event) {
...
var hasNoMarker = function hasNoMarker(event) {
...
var markBoard = function markBoard(event) {
...
var onUpdateGame = function onUpdateGame(event) {
  if (over === false && hasNoMarker(event) === true) {
    ...
    api.update(game).then(ui.updateBoardSuccess).then(markBoard(event)).catch(ui.updateBoardFailure);
  }
};
...
module.exports = {
  ...
  displayGameStat: displayGameStat,
...
var createBoardFailure = function createBoardFailure() {
  // console.error(error)
...
var updateBoardFailure = function updateBoardFailure() {
  // console.error(error)
  $('#message').text('Oops! Something went wrong. Please try again.');
};
